...
export default function ArtistSidebar({ className }: ArtistSidebarProps) {
    ...
    return (
        <div className={`bg-[#172554] text-white flex flex-col h-screen transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'} ${className}`}>
            ...
            <div className="p-4 border-b border-gray-700">
                <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-end'} `}>
                    ...
                    <Button
                        ...
                        className={`w-full justify-start py-2 text-sm font-medium transition-colors text-white-300 hover:bg-[#334155] hover:text-white ${isCollapsed ? 'px-2' : 'px-3'}`}
                    >
            ...
            {!isCollapsed && (
                <div className="p-4 pt-8 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                    CONTENT MANAGEMENT
                </div>
            )}
            ...
            {/* Navigation Menu */}
            <nav className="flex-1 p-4 space-y-1 h-full">
                {menuItems.map((item) => (
                    <Button
                        key={item.path}
                        variant="ghost"
                    ...
                    >
                        <item.icon className={`${isCollapsed ? 'w-5 h-5' : 'w-4 h-4 mr-3'} text-white-500`} />
                        {!isCollapsed && (
                            <>
                                <span className="text-white-700 light:text-white-300">{item.label}</span>
                                {item.badge && (
