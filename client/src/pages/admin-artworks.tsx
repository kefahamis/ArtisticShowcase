...
const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL || "https://api.talantaart.com";
...
export default function AdminArtworks() {
  ...
  const authenticatedRequest = async (
    ...
    const response = await fetch(
      url.replace(/^\/api/, API_BASE_URL + "/api"),
      {
        method,
        headers: {
  ...
  // Handle image URLs for rendering with fallback
  ...
  // Map artworks to include processed image URLs for rendering
  ...
  // Fetch artists
  const { data: artists = [], error: artistsError } = useQuery({
  ...
  useEffect(() => {
    if (artworksError || artistsError) {
      toast({
        title: "Error",
        description:
          (artworksError || artistsError)?.message || "Failed to load data",
        variant: "destructive",
      });
    }
  }, [artworksError, artistsError, toast]);
  ...
  const updateArtworkMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<ArtworkForm> }) => {
      const payload = {
        ...
        price: data.price ? parseFloat(data.price) : undefined, // Convert to number
      };
  ...
  const {
    ...
    endItem,
    totalItems,
  } = usePagination<Artwork>({ data: processedArtworks, itemsPerPage: 10 });
        ...
        <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4">
          ...
          <div className="flex-1 flex items-center justify-between">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              {/* FIXED: Corrected the z-index and removed invalid class */}
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            >
                              ...
                              {/* FIXED: Ensured SelectContent is above DialogContent */}
                              <SelectContent className="z-[60]">
                                {artists && artists.length > 0 ? (
                                  ...
                                  <SelectItem value="no-artists" disabled>
                                    No artists available. Create an artist
                                    first.
                                  </SelectItem>
                                )}
                              </SelectContent>
                            ...
                            <FormControl>
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  field.onChange(value.toString()); // Keep as string in form state
                                }}
                              />
                            </FormControl>
                            <FormMessage />
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Artworks (
                {Array.isArray(processedArtworks)
                  ? processedArtworks.length
                  : 0}
                )
              </CardTitle>
            </CardHeader>
                      ...
                      <TableRow key={artwork.id}>
                        ...
                        <TableCell>
                          ...
                          >
                            ...
                            {artwork.availability.charAt(0).toUpperCase() +
                              artwork.availability.slice(1)}
                          </Badge>
                        </TableCell>
              ...
              {Array.isArray(processedArtworks) &&
                ...
                !loadingArtworks && (
                  <div className="text-center py-8 text-gray-500">
                    No artworks found. Create your first artwork to get
                    started.
                  </div>
                )}
              {/* Pagination */}
