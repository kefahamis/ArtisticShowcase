...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/artists/register", async (req, res) => {
    try {
      ...
      // Register artist with user account
      ...
      res.status(201).json({ 
        ...
        token,
        user: { id: user.id, email: user.email, username: user.username },
        artist: { id: artist.id, name: artist.name, bio: artist.bio, specialty: artist.specialty }
      });
    } catch (error: any) {
      res.status(400).json({ message: error.message });
    }
  ...
  app.put("/api/artists/profile", authenticateArtist, async (req: any, res) => {
    try {
      const updateData = insertArtistSchema.partial().parse(req.body);
      const updatedArtist = await storage.updateArtist(req.artist.id, updateData);
  ...
  app.get("/api/artists/orders", authenticateArtist, async (req: any, res) => {
    try {
      const orders = await storage.getArtistOrdersForArtworks(req.user.id);
      res.json(orders);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
